...
const AFTER_HOURS_WORDS = /((24\/7|24 hours|around the clock|available anytime|nights and weekends)[^.!?]{0,60}(answer|call|phone|reach|staff|team|support|service|open|available|assist|help)|(answer|call|phone|reach|staff|team|support|service|open|available|assist|help)[^.!?]{0,60}(24\/7|24 hours|around the clock|after hours|nights and weekends)|after[- ]hours (answer|call|phone|service|line|support|coverage)|emergency (service|line|call|number))/i;
const AFTER_HOURS_EXCLUDE = /(portal|app|online (access|account|ordering|booking|store)|self[- ]serve|website|dashboard|financing|loan|payment plan|chatbot)/i;
...
export async function researchLead(lead, { pages = PAGES, timeoutMs = 15000, fetchImpl = globalThis.fetch } = {}) {
  ...
  for (const { path, label } of pages) {
    ...
    if (!result.ok || !result.html) {
      evidence.pages_failed.push({ url: result.url, status: result.status, error: result.error ?? null });
      continue;
    }
    if (path === '/') { homepageHtml = result.html; evidence.website_reachable = true; }
    evidence.pages_read.push({ url: result.final_url ?? result.url, label });
    evidence.evidence_sources.push(result.final_url ?? result.url);
    extractInto(evidence, result.html, result.final_url ?? result.url, label);
  }

  // Deduplicate accumulated lists.
